...
@protocol TextFieldValidatorSupportProtocol
...
@property (nonatomic,retain) id<TextFieldValidatorSupportProtocol> delegateTextFieldValidatorSupportProtocol;
@end

@implementation TextFieldValidatorSupport
@synthesize delegate,validateOnCharacterChanged,popUp,validateOnResign;
@synthesize delegateTextFieldValidatorSupportProtocol;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField.tag == 1) {
        ...
        image = [CTSUtility getSchmeTypeImage:[[textField.text stringByReplacingOccurrencesOfString:@" " withString:@""] stringByAppendingString:string]];
        [delegateTextFieldValidatorSupportProtocol getSchmeTypeImage:image];
    }


    // Card number
    if (textField.tag == 1) {
        return [CTSUtility appendBlanckSpaceForCardnumber:textField replacementString:string shouldChangeCharactersInRange:range];
    }

    
    // Expiry Date
    if (textField.tag == 2) {
        return [CTSUtility appendBlanckSpaceForExpiryDate:textField replacementString:string shouldChangeCharactersInRange:range];
    }
...
@interface TextFieldValidator()<TextFieldValidatorSupportProtocol>{
...
@synthesize delegateTextFieldValidator;
...
-(void)setup{
    ...
    supportObj.delegateTextFieldValidatorSupportProtocol = self;
...
- (UIImage*)getSchmeTypeImage:(UIImage*)image{
    return [delegateTextFieldValidator getSchmeTypeImage:image];
}
