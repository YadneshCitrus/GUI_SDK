...
+ (BOOL)appendBlanckSpaceForCardnumber:(UITextField*)textField replacementString:(NSString*)string shouldChangeCharactersInRange:(NSRange)range{
    // Reject appending non-digit characters
    if (range.length == 0 &&
        ![[NSCharacterSet decimalDigitCharacterSet]
          characterIsMember:[string characterAtIndex:0]]) {
...
+ (BOOL)appendBlanckSpaceForExpiryDate:(UITextField*)textField replacementString:(NSString*)string shouldChangeCharactersInRange:(NSRange)range{
    // Reject appending non-digit characters
    if (range.length == 0 &&
        ![[NSCharacterSet decimalDigitCharacterSet]
          characterIsMember:[string characterAtIndex:0]]) {
    ...
    // Auto-add space before appending 2nd digit
    if (range.length == 0 &&
        (range.location == 2)) {
        textField.text =
        [NSString stringWithFormat:@"%@      %@", textField.text, string];
